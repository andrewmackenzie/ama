...
/// Manage ama's LaunchAgent so the daemon starts at login.
///
/// We deliberately do NOT use SMAppService.mainApp here — that requires a full
/// .app bundle. Since ama ships as a single binary in /usr/local/bin, a
...
struct Install: ParsableCommand {
    ...
    @Flag(name: .long, help: "Register ama to start at login.")
    ...
    private static let label = "com.capstannetworks.ama"
    ...
    private func writeAgent() throws {
        ...
        let plist: [String: Any] = [
            ...
            "StandardOutPath": "/tmp/ama.out.log",
            "StandardErrorPath": "/tmp/ama.err.log",
        ]
        ...
        print("  binary: \(binary)")
        print("  logs:   /tmp/ama.out.log, /tmp/ama.err.log")
    }
    ...
    private func resolveBinaryPath() throws -> String {
        // /usr/local/bin/ama is the canonical install path. Honor a real
        // location if running from elsewhere (e.g. dev).
        let candidate = "/usr/local/bin/ama"
        if FileManager.default.isExecutableFile(atPath: candidate) {
            return candidate
        }
        // Fall back to the running executable's resolved path.
        let argv0 = CommandLine.arguments.first ?? "ama"
        if argv0.hasPrefix("/"), FileManager.default.isExecutableFile(atPath: argv0) {
            FileHandle.standardError.write(Data(
                "note: /usr/local/bin/ama not found; using \(argv0)\n".utf8
            ))
            return argv0
        }
        FileHandle.standardError.write(Data(
            "couldn't locate the ama binary. install it to /usr/local/bin/ama first.\n".utf8
        ))
        throw ExitCode(1)
    }
